...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = err.message || 'Login gagal. Periksa kembali email dan password Anda.';
      setError(msg);
      toast.error(msg);
    } finally {
      setLoading(false);
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-card border border-white/60">
          ...
          <button
            ...
            className="w-full py-3 px-4 bg-white border border-gray-200 hover:bg-gray-50 active:scale-95 text-gray-700 font-bold text-xs rounded-xl shadow-sm flex items-center justify-center gap-3 transition-all mb-4"
          >
            ...
            <span>{oauthLoading ? 'Menghubungkan...' : 'Masuk dengan Google'}</span>
          </button>

          {/* Divider */}
          <div className="relative flex items-center justify-center mb-4">
            <div className="border-t border-gray-200 w-full" />
            <span className="bg-white px-3 text-[11px] text-gray-400 font-semibold absolute">
              atau dengan email
            </span>
          </div>
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            Belum punya akun?{' '}
            <Link to="/reset-password" className="text-gray-400 text-xs hover:underline block mb-2 sm:hidden">
              Lupa password?
            </Link>
